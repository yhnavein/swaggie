...
| `--responseShape` | — | `string` | — | Standardize operation return shape across templates: `body` returns the response body (`T`); `full` returns an `APIResponse<T>` wrapper with `data`, `headers` and `statusCode`. Omitted = each template keeps its current default shape. See [Response shape](/guide/configuration#responseshape). |
